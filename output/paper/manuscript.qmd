...
Neurodevelopmental outcomes were assessed with standardized, non-linguistic, and culturally blind computer tests, including the {{< acr ant first_use=true >}} [@RuedaFanMcCandliss:2004], which provides a measure of efficiency in three different functions of attention: alerting, orienting, and executive attention. The tests were administered in a standardized way, and with minimal interference from the field workers. Further information can be found in @MaitreBontCasas:2018. The outcome of interest for the present study is the {{< acr hitrtse first_use=true >}} [@SunyerEsnaolaAlvarez-Pedrerol:2015], a measure of response speed consistency throughout the test. A high {{< acr hitrtse >}} indicates highly variable reactions, and is considered a measure of inattentiveness.
...
@Fig-marginal-1 presents the forest plot for the {{< acr mcs >}} of the non-persistent {{< acr edcs >}} on {{< acr hitrtse >}}. For most {{< acr edcs >}}, a cohort-specific decrease in the levels of the exposures from the 90th to the 10th percentiles was associated with a positive {{< acr mc >}}, indicating an increase in the values of {{< acr hitrtse >}}. Most of the {{< acr cis first_use=true >}} included the null effect, though. Statistically significant effects were observed for the phenol MEPA (`r format_res(meffects1, "mepa", "HRT-SE")`), and the phthalate metabolites oxo-MiNP (`r format_res(meffects1, "oxominp", "HRT-SE")`), oh-MiNP (`r format_res(meffects1, "ohminp", "HRT-SE")`), and MEHP (`r format_res(meffects1, "mehp", "HRT-SE")`). The {{< acr opp >}} DETP was negatively associated with {{< acr hitrtse >}} (`r format_res(meffects1, "detp", "HRT-SE")`).
...
We decided to model both the *treatment* mechanisms, for the estimation of balancing weights, and the outcomes, with traditional covariates adjustment, to try to obtain *doubly robust* effect estimates. Finally, we decided not to interpret our results by focusing on the estimated coefficients of the regression models, but by making use of the g-computation procedure and estimate {{< acr mcs >}}.
